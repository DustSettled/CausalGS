...
class GUI:
    def __init__(self, args, dataset, opt, pipe, testing_iterations, saving_iterations, n_keys, vel_start_time=0.0) -> None:
        ...
        if args.use_mpm:
            try:
                ...
                mpm_config.grid_resolution = (args.mpm_grid_res, args.mpm_grid_res, args.mpm_grid_res)
                mpm_config.dt = 1.0 / args.fps
                print(f"🔧 MPM Physics enabled:")
                print(f"   Grid resolution: {mpm_config.grid_resolution}")
                print(f"   Physics weight: {args.mpm_weight}")
            except ImportError:
                print("⚠️  Warning: MPM module not available (warp-lang not installed)")
                print("   Continuing without physics constraints")
                print("   To enable: pip install warp-lang")
                args.use_mpm = False  # 静默降级

        self.deform = DeformModel(
            max_time=dataset.max_time,
            vel_start_time=vel_start_time,
            light=dataset.light,
            physics_code=dataset.physics_code,
            use_mpm=args.use_mpm,
            mpm_config=mpm_config,
            use_velocity_field=not args.disable_velocity_field  # Ablation control
        )
    ...
    def train_step(self):
        ...
        render_loss = (1.0 - self.opt.lambda_dssim) * Ll1 + self.opt.lambda_dssim * (1.0 - ssim(image, gt_image))

        # ============ MPM Physics Loss ============
        if self.deform.use_mpm and self.iteration > self.opt.warm_up:
            # Get deformed positions
            xyz_deformed = self.gaussians.get_xyz + d_xyz

            # Compute physics regularization loss
            physics_loss = self.deform.get_mpm_regularization_loss(
                xyz=xyz_deformed,
                weight=self.args.mpm_weight
            )

            # Combine losses
            loss = render_loss + physics_loss

            # Log physics loss to TensorBoard
            if self.tb_writer and self.iteration % 10 == 0:
                self.tb_writer.add_scalar('train_loss/render', render_loss.item(), self.iteration)
                self.tb_writer.add_scalar('train_loss/physics', physics_loss.item(), self.iteration)
                self.tb_writer.add_scalar('train_loss/total', loss.item(), self.iteration)
        else:
            loss = render_loss
        # ==========================================
        ...
        with torch.no_grad():
            # Calculate PSNR for current iteration
            ...
            # if self.tb_writer:
            #     self.tb_writer.add_scalar('train_iter/loss', loss.item(), self.iteration)
            #     self.tb_writer.add_scalar('train_iter/psnr', train_psnr.item(), self.iteration)
            #     self.tb_writer.add_scalar('train_iter/l1_loss', Ll1.item(), self.iteration)
...
if __name__ == "__main__":
    ...
    # MPM Physics Engine arguments
    parser.add_argument('--use_mpm', action='store_true', help='Enable MPM physics engine')
    parser.add_argument('--mpm_weight', type=float, default=0.1, help='Weight for MPM physics loss')
    parser.add_argument('--mpm_grid_res', type=int, default=64, help='MPM grid resolution (power of 2 recommended)')

    # Ablation study arguments
    parser.add_argument('--disable_velocity_field', action='store_true',
                       help='Disable velocity field for ablation study (DeformNetwork + MPM only)')
